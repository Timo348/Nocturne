...
export default function WidgetFrame({ widget, definition, editing, onEdit, onDelete, onKeyboardLayout }: Props) {
  ...
  }, [widget.id, widget.updatedAt]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <article className={`widget-frame ${editing ? "is-editing" : ""}`} data-widget-id={widget.id} data-widget-type={widget.type} data-widget-title={widget.title} style={{ "--widget-accent": definition.accent } as React.CSSProperties}>
      ...
      <header className="widget-header">
        ...
        <div className="widget-actions">
          ...
          {menu && <div className="widget-menu"><button onClick={() => { setMenu(false); void load(); }}><RefreshCw size={14} /> Aktualisieren</button><button onClick={() => { setMenu(false); onEdit(); }}><Settings2 size={14} /> Konfigurieren</button>{editing && <button className="danger-item" onClick={() => { setMenu(false); onDelete(); }}><Trash2 size={14} /> Entfernen</button>}</div>}
        </div>
      </header>
